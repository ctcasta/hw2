...
#count of hospitals/provider_number by year in class code
dup.count <- finalhcris %>%
    add count(provider number, fyear, name='total_reports') %>%
    mutate(dup_reports = (total_reports>1))%>%
    group_by(year) %>%
    summarize(duplicates=sum(dup_reports))
...
final_hcris['fyera']=final_hcris['fy_end'].dt.year

#count distinct providers
num_providers = hcris_data